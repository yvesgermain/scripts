...
get-printer -ComputerName oldServer | where {$_.shared -eq $true} | select name,DriverName,PortName,location | export-csv <path>\newserver.csv -Encoding unicode -Delimiter ","
You have to clean up the CSV file.  Correct the Printer names (No spaces, Please!).  The prefix of the printer names will be generated by the script.  
